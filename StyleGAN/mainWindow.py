...
def Canvas_(root):
    ...
    # 创建标签
    label = tkinter.Label(root, text='随机生成图像', font=('黑体', 14), width=15, height=1)
...
def generagedImage(model):
    ...
    steps = 5
    genImg = model(config.FIXED_NOISE,alpha,steps).detach().cpu().numpy()
    genImg = np.squeeze(genImg)
    fig = plt.figure(figsize=(2,4))
    for i in range(8):
        plt.subplot(2,4, i + 1)
        # 由于生成器输出的结果为[-1,1]之间，所以需要转换为[0,1]之间
        plt.imshow(np.transpose((genImg[i] + 1) / 2))
        plt.axis('off')
    plt.savefig('cur.png')
...
if __name__ == '__main__':
    ...
    model = Generator(
        config.Z_DIM, config.W_DIM, in_channels=config.IN_CHANNELS,img_channels=config.CHANNELS_IMG
    ).to(config.DEVICE)
    checkpoint = torch.load(f = 'models/generator.pth',map_location='cpu')
