...
def generagedImage(model):
    ...
    :return:
    """
    noise = torch.randn(size=(4, config.LATENT_DIM,1,1)).to(config.DEVICE)
    # fixed labels
    y_ = (torch.rand(4, 1) * config.NUM_CLASSES).type(torch.LongTensor)
    y_fixed = torch.zeros(4, config.NUM_CLASSES)
    y_fixed = y_fixed.scatter_(1, y_.view(4, 1), 1).view(4,config.NUM_CLASSES,1,1)
