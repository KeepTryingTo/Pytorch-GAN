...
for epoch in range(config.NUM_EPOCHS):
    ...
    for step ,data in enumerate(loop):
        ...
        batchSize = np.shape(imgs)[0]
        channels = np.shape(imgs)[1]
        imgSizeW = np.shape(imgs)[2]
        imgSizeH = np.shape(imgs)[3]
        #在原始图像上添加高斯噪声mean = 0,std = 1
        gaussionNoise = torch.randn(size=(batchSize,channels,imgSizeW,imgSizeH))
        imgs = torch.add(input=imgs,other=gaussionNoise)
