...
def train_fn(generator,discriminator,optimizer_G,optimizer_D,adversarial_loss,dataloader,epoch):
    ...
    for step,(imgs,labels) in enumerate(loop):
        valid = torch.ones(config.BATCH_SIZE)
        fake = torch.zeros(config.BATCH_SIZE)
        ...
        real_y = real_y.scatter_(1, labels.view(config.BATCH_SIZE, 1), 1)

        noise = torch.randn(size = (config.BATCH_SIZE,config.LATENT_DIM)).to(config.DEVICE)
        ...
        # dim = 1,index = config.BATCH_SIZE,src = 1
        gen_y = gen_y.scatter_(1, gen_labels.view(config.BATCH_SIZE, 1), 1)
        ...
        gen_imgs = generator(noise,real_y)
        # d_fake_loss = adversarial_loss(np.squeeze(discriminator(gen_imgs.detach(),gen_y)),fake)
        ...
        # g_loss = adversarial_loss(np.squeeze(discriminator(gen_imgs,gen_y)),valid)
