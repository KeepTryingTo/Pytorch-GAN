...
def generagedImage(model):
    ...
    :return:
    """
    random_noise = torch.randn(size=(16,config.Z_DIM,1,1), device='cpu')
    alpha = 1e-5  # start with very low alpha
    steps = 5
    genImg = model(random_noise,alpha,steps).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(f = 'models/generator_128.pth',map_location='cpu')
